...
**Agent 面（②）**：网关把 `/v1/agent/*` 转发到 PilotDeck `api_server`（默认 `127.0.0.1:8642`，由 `API_SERVER_HOST/PORT/KEY` 配置），对外用 `X-Session-Id`（内部翻译成会话键 `{app_id}:{session_id}:{gen}`，经 `X-Hermes-Session-Id` 传入），对外隐藏 runtime 细节。**转发之上是网关增值层（§6.3）**：会话登记/回放、`background` 异步聚合、逻辑删除、并发闸门——runtime 只做单回合执行，② 的产品形态由网关补齐，不依赖上游排期。agent 回调 ① 做推理是同设备模型线调用，不引入计费语义。详见 `interfaces/agent-api.md`。
...
每个 backend 实现统一接口：输入 OpenAI 形状 → 调后端进程 → 输出 OpenAI 形状。加模态/换后端 = 加一个 adapter + 路由表一行，**SI 侧无感**。**网关代码量唯一会增长的地方是 backends/**，可控。同一能力可有多个 adapter（如 STT 的 speaches / vllm_omni），按盒子配置在路由表里选。
...
**SI 容器怎么访问 Gateway（标准部署方式）**：容器默认网络下访问不到宿主机的 `127.0.0.1`，"容器经 127.0.0.1 调网关"只在 host network 模式下成立。两种部署方式二选一：
...
不要为了图省事直接绑 `0.0.0.0` 且不开门锁；交付脚本应默认方式 1。
...
- **升级流程（MVP 不做 OTA）**：定义现场脚本升级网关与 skills（不动模型）的标准程序；`/healthz` 带 `gateway_version` 供远程确认版本。
