...
**Agent 面（②）**：网关把 `/v1/agent/*` 转发到 PilotDeck `api_server`（默认 `127.0.0.1:8642`，由 `API_SERVER_HOST/PORT/KEY` 配置），对外用 `X-Session-Id`（内部翻译成 `X-Hermes-Session-Id`），藏掉 runtime 细节。其内部 agent 回打 ① 做推理是同设备模型线调用，不引入额外商业计费语义。会话读/删（`GET .../sessions/{id}/messages`、`DELETE .../sessions/{id}`）与异步长回合（`background:true` → `GET /v1/agent/turns/{id}`）见 `interfaces/agent-api.md` §3/§4。
...
启停只改 registry + runtime 可见性，不修改 `SKILL.md` 内容。删除先禁用再移除目录；失败时保持 registry 和磁盘一致。详见 `interfaces/skill-contract.md` §2。

### 6.3 Agent 面默认运行边界

- `X-Session-Id` 缺省时由 Gateway 生成并回带；SI App 应为每个最终用户/业务对象维护稳定 session。有效会话 = `(X-Pinea-App-Id, X-Session-Id)`（app-id 缺省 `default`，多 SI 隔离 v1 预留）。
- 默认 workspace 使用设备的 ToB 工作区（部署时配置，例如 `/srv/pinea/workspaces/default`），不得直接暴露系统根目录。
- 同一 session 同时只允许一个进行中回合；并发返回 `429`。
- Agent 面可以使用已安装 Skills / tools / MCP，但工具权限由 runtime 配置收口；Gateway 不绕过 runtime 权限模型。该权限边界经 `/v1/capabilities` 的 `agent.{tools,permissions}` 对 SI **只读可见**。
- 会话可经 `DELETE /v1/agent/sessions/{id}` 删除（连带清记忆作用域，被遗忘权抓手）；MVP 不自动淘汰。
