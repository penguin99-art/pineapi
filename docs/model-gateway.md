...
一个 OpenAI 兼容的多模态能力网关，是**三类 ToB 的唯一对外前门**（一个 base_url + 一个 key），**只做三件事**：**路由 + schema 归一 + 治理（鉴权/配额/计量/日志/健康）**。
...
SI 集成方   +   PilotDeck(model.providers 指向网关)
        │  一个 base_url · 一套 key · OpenAI 兼容（三类 ToB 同一前门）
        ▼
┌─────────────────────────────────────────────────────────────────┐
│  Pinea Model Gateway (FastAPI · 薄) — 唯一对外前门                  │
│  路由表 capability → backend adapter                               │
│  统一: 鉴权/配额/计量(MVP 留桩) · 日志 · /v1/capabilities 发现 · 健康 │
├──────────┬──────────────┬───────────┬───────────┬──────────────┤
...
└──────────┴──────────────┴───────────┴───────────┴──────────────┘
   ...
   ②转发 PilotDeck api_server;其内部回 model.providers 打①做推理时标 internal,不重复计量
   ③Skills 由②的 agent 自动触发(磁盘 skill 扩展点)
```
...
**Agent 面（②）**：网关把 `/v1/agent/*` 转发到 PilotDeck `api_server`，对外用 `X-Session-Id`（内部翻译成 `X-Hermes-Session-Id`），藏掉 runtime 细节。其内部 agent 回打 ① 做推理的调用标 internal，**只计 ② 回合不重复计 ① tokens**。
...
│  ├─ agent.py            # ② Agent 面: 转发 PilotDeck api_server, session 头翻译
...
- **配额/计量**：MVP 在 `auth.py` 留计量钩子（① tokens/秒数/张数、② agent 回合，统一 usage 形状见 `interfaces/capability-api.md` §8），不强制限额；后置接计费。② 内部回环调用标 internal，不重复计。key 标作用域（买了①②③哪些）。
