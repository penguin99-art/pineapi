...
| **③ L7 公共 Skill 契约** | 我们的管理接口 + PilotDeck（SKILL.md） | 行业交付 · SI | [`skill-contract.md`](skill-contract.md) | **ToB 第三类**：卖可复用行业能力 |
...
1. **对外契约（ToB，最稳）**：能力面 API + Agent 面 API + 公共 Skill 契约。**`v1.0` 已冻结（2026-06-10）**——形状是开发权威基线，破坏走版本。2026-06-11 修订：② 会话/异步端点重判为**网关侧实现**并升为 Stable·自定义（只增强承诺，不破坏冻结）。每端点档位 + 实现状态见 [`tob-overview.md`](tob-overview.md) §7 矩阵。OpenAI 兼容子集承诺永不破。
...
- 不改 PilotDeck 核心：能力面在 runtime *下面*（model.providers），skill 对外走 Gateway 管理接口、内部落 runtime *磁盘扩展点*，全是官方扩展点。
...
- **③ 公共 Skills**：照 `skill-contract.md` 写 Skill 包（核心 `SKILL.md`），通过 Gateway 管理接口安装/启停；先用能力面 mock 联调，不等真模型。
