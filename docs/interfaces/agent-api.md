...
- **删除 = 不可达 + 网关侧清除**：① 会话键 `gen` +1，旧上下文/记忆对该 session 永久不可达；② 清除网关侧登记与回放数据。删除成功后重复调用不会让旧会话复活；若会话不存在，返回 `404 session_not_found`。**runtime 侧旧数据的物理擦除是运维程序**（交付手册：停 runtime 清理数据目录），原生支持为 Reserved·需上游。SI 对最终用户的"删除/被遗忘"承诺应按此措辞：立即不可达，物理擦除随运维周期。
...
- ② 内部 agent 经 `model.providers` 调用 ① 做推理，属同设备模型线调用，不引入计费语义。
