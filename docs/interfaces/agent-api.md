...
1. **流式（推荐）**：`stream:true`，SSE 长连边跑边回（见 §2），SI 拿到 `[DONE]` 即结束。
2. **异步提交/轮询**（无法长连时）：请求体加 Pinea 扩展字段 `"background": true`：
...
| 删会话 | `DELETE /v1/agent/sessions/{id}` | 删除会话及其记忆作用域；幂等（不存在回 `404` `session_not_found`） |
