...
> 这是对外卖给 B 端（SI 集成方 / 行业交付）的接口**唯一入口**。详细契约见各层链接。
...
三类 ToB **共用 Pinea Model Gateway 一个前门**（一个 `base_url` + 一个 `key`），自底向上叠，按"自己搭多少 vs 白嫖多少"选层：

```
B 端开发者 ── 一个 base_url + 一个 key ──► Pinea Model Gateway（唯一对外前门 · 统一鉴权/计量/发现/日志）
   │
   ├─ ③ Skills 面 (L7)   = ② + 行业流程封装。连流程都不用写，丢个 SKILL.md
...
| L7 | ③ Skills | 可复用行业能力 | 磁盘 `SKILL.md`，由②自动触发 | — | 作业次 | [`skill-contract.md`](skill-contract.md) |

## 1. 共用约定（三类统一，定义"好"的关键）

- **前门**：一个 `base_url`（默认 `:8080`，待定）；所有 HTTP 端点前缀 `/v1`。
- **鉴权**：`Authorization: Bearer <key>`。一个客户一个 key，key 标了买了哪几类（①/②/③）。未购买的类打对应端点 → `403`。
- **能力发现**：`GET /v1/capabilities` 一次回三类（哪些模态可用 / agent 面开没开 / 装了哪些 skill），受 key 作用域过滤。盒子硬件不同，B 端必须程序化可查。
- **错误形状**（OpenAI 风格，全端点统一）：
  ```json
  { "error": { "message": "...", "type": "invalid_request_error|server_error|rate_limit_error", "code": "optional" } }
  ```
- **计量**：三类写**同一条结构化用量日志**，单位不同形状统一（见 `capability-api.md` §8）。② 内部回环推理标 internal，不重复计。
- **稳定性**：均为对外契约，破坏性变更走版本（`v1`→`v2` + 弃用周期）。**OpenAI 兼容子集承诺永不破**。当前 `v1` draft 未冻结。
...
| 端点 | 说明 |
| --- | --- |
| `POST /v1/agent/chat/completions` | OpenAI chat 兼容 + `X-Session-Id` 串上下文+记忆；agent 自用工具、自己多步循环，一次出最终结果 |
...
- 形态：一个目录 + 一个 `SKILL.md`（YAML frontmatter `name`/`description` + markdown 操作手册）+ 可选脚本。
- 加载：runtime 扫描磁盘 skill 目录，② 的 agent 按 `description` 自动触发。**零改核心**。
- 调能力：skill 通过 ① 能力面端点拿多模态（经注入的 `PINEA_GATEWAY`/`PINEA_KEY`，不写死地址）。
...
- **不改 PilotDeck 核心**：① 在 runtime 下面（model.providers）；② 复用 api_server 官方扩展点；③ 走磁盘 skill 扩展点。全是官方扩展点。
