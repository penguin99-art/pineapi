...
- **安全门锁**：`Authorization: Bearer <token>`。本机封闭部署可关闭；只要 Gateway 绑定到 LAN/WAN 就必须开启。Token 只解决"谁能调设备口"，不表达购买范围。
- **能力发现**：`GET /v1/capabilities` 一次回三类（哪些模态可用 / agent 面开没开 / 装了哪些 skill）。盒子硬件不同，B 端必须程序化可查。
...
- 安装：SI / 我们通过 Gateway 本机管理接口（如 `/v1/skills/install`）安装/启停；内部落到 runtime skill 目录，② 的 agent 按 `description` 自动触发。**零改核心**。
