...
> 配套：设计见 `../model-gateway.md`，总览见 `README.md`。
> 这是 **ToB 第一类接口**——SI 集成方照此对接，PilotDeck 也经 `model.providers` 消费同一份。
> **稳定性**：对外契约，破坏性变更走版本（见 §版本）。当前 = `v1`（draft，未冻结）。

## 0. 约定

- **Base URL**：`http://<host>:<port>`（默认 `:8080`，待定）。所有端点前缀 `/v1`。
- **鉴权**：`Authorization: Bearer <key>`。MVP 单 key；多租户后置（见 `../model-gateway.md` §6）。
...
- **能力发现**：`GET /v1/models` 聚合所有后端可用模型；`GET /healthz` 聚合各后端探活。
...
- `GET /v1/models` → `{ "object":"list","data":[{"id","object":"model","owned_by"}] }`（跨后端聚合，标各自 capability）。
