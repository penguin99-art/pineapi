...
uv run python -m pinea_gateway               # 起在 :18800
```

冒烟：

```bash
curl -s localhost:18800/healthz
curl -s localhost:18800/v1/chat/completions \
...
PINEA_TEST_BASE_URL=http://localhost:18800 uv run pytest   # 打真网关，同一套用例
```
...
```bash
# deploy/pilot-home/pilotdeck.yaml: model.providers.ollama.url 改为 http://localhost:18800/v1
