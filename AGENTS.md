...
模型层 = 自研 **Pinea Model Gateway**（OpenAI 兼容能力面：chat→ollama / STT / TTS / 生图 / 视频；只路由+归一+治理，不做推理；详见 `docs/model-gateway.md`）；LocalAI 降级为可选后端，不再当门面。执行内核 = **Agent Runtime 层**，当前实现 = PilotDeck（自有，可商用可改）；自研只依赖其契约/扩展点，故 runtime 原则上可换（见 `docs/architecture.md` §0）。
...
1. **风险前置。** 最大不确定性是「本地模型能否稳定跑 agentic tool calling」。先死磕 LocalAI + MiniCPM 的 tool calling，再做其他。别先做灯效。
...
4. **知识注入先于动手**：写哪块前，先把权威源（PilotDeck 源码、MiniCPM/LocalAI/MCP 文档）喂给 agent 蒸成 `research/knowledge/` 的 markdown，再实现。
5. **约束驱动**：把盒子真实约束（功耗 / NPU / 端侧延迟）喂进去，让方案只在可行域里探索，别选端侧跑不动的设计。
6. **风险前置即 spike**：build-plan 第 0 步（LocalAI+MiniCPM tool calling）就是第一个 spike，见 `research/spikes/tool-calling-localai.md`，过了再写集成代码。
